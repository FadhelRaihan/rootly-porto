...
import { localizedField, localizedFieldArr } from '@/lib/lang-utils'
...
export default async function ServicesPage() {
  ...
  return (
    <div className="pt-20 bg-rootly-background min-h-screen text-rootly-text">
              ...
              <AnimatedSection key={service.id} delay={index * 0.1}>
                <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 lg:gap-16 items-center">
                  ...
                  <div className={`lg:col-span-6 ${index % 2 === 1 ? 'lg:order-2' : ''}`}>
                    <div className="flex items-center gap-2 mb-4">
                      <span className="font-mono text-[9px] text-rootly-primary tracking-widest uppercase">{`[ SERVICE // ${protocolNum} ]`}</span>
                    </div>
                    
                    <h2 className="text-3xl font-serif text-rootly-text mb-4">{localizedField(language, service.title, service.titleId)}</h2>
                    <p className="text-rootly-muted text-sm leading-relaxed mb-6">{localizedField(language, service.description, service.descriptionId)}</p>
                      ...
                      <ul className="space-y-2">
                        {localizedFieldArr(language, service.useCases, service.useCasesId).map((useCase: string) => (
                          <li key={useCase} className="text-xs text-rootly-muted flex items-start gap-2.5">
                            <span className="w-1.5 h-1.5 bg-rootly-primary rounded-full shrink-0 mt-1.5" />
                    ...
                    <Link href="/contact" className="inline-flex items-center gap-2 text-rootly-primary font-mono text-xs tracking-wider uppercase hover:underline group">
                  ...
                  <div className={`lg:col-span-6 ${index % 2 === 1 ? 'lg:order-1' : ''}`}>
                    <div className="bg-rootly-deep-bg text-gray-300 rounded-2xl p-6 border border-rootly-border/60 shadow-xl font-mono text-xs relative overflow-hidden hover:border-rootly-primary/40 transition-all duration-300">
                      ...
                      <div className="space-y-4">
                        <div className="flex items-center gap-4">
                          <div className="w-12 h-12 bg-white/5 border border-white/10 rounded-xl flex items-center justify-center text-rootly-primary">
                        ...
                        <div className="grid grid-cols-2 gap-3 py-3 border-y border-dashed border-rootly-border/60">
                          {service.icon === 'Globe' && (
                            <>
                              <div>
      ...
      <section className="py-24 bg-rootly-background relative overflow-hidden border-t border-dashed border-rootly-border">
        ...
        <div className="max-w-7xl mx-auto px-4 md:px-8 relative z-10">
          <AnimatedSection>
            <div className="text-center mb-16">
              <span className="text-[10px] font-mono tracking-widest text-rootly-primary uppercase block mb-3">{"[ SYSTEM REGISTRY // TECH PACKAGES ]"}</span>
              <h2 className="text-3xl font-serif text-rootly-text text-center mb-4">Technologies We Use</h2>
              <p className="text-rootly-muted text-xs font-mono">SUPPORTED DEVELOPMENT PROTOCOLS ON MAINNET</p>
            </div>
          </AnimatedSection>
          ...
          <div className="flex flex-wrap justify-center gap-3 max-w-4xl mx-auto">
            {techList.map((tech) => (
              <span 
                key={tech.id} 
                className="px-4.5 py-2 bg-rootly-surface border border-rootly-border rounded-xl text-xs font-mono text-rootly-text flex items-center gap-2 hover:border-rootly-primary/45 hover:shadow-xs transition-all duration-200 cursor-default"
              >
